...
(defun cider-scratch-project--create-buffer ()
  ...
  (with-current-buffer (get-buffer-create (cider-scratch-project--buffer-name))
    (cider-clojure-interaction-mode)
    ;; Prompt to persist unsaved changes when this project scratch is killed.
    ;; Scoped here (rather than in `cider-clojure-interaction-mode') so the
    ;; session scratch buffers, which share that mode, aren't affected.
